#########################################################################################
#### Trial Design Parameters for platform trials with unequal allocation (Scenario A)####
#########################################################################################
...
patients_per_experimental_arm <- 45 # Total number of patients in each treatment arm
death0 <- 0.41 # Mortality rate for the control arm
death1 <- 0.36 # Mortality rate for the treatment arms
eath_probs = c(0.41, 0.36, 0.36, 0.36, 0.36)  # Mortality rate for the control and 4 treatment arms
true_rd <- death1 - death0 # True risk difference

ni_margin <- 0.20 # non-inferiority margin for scenario A  
death_alt <- death0 + ni_margin  # Experimental treatment is inferior (worse outcome rate) for the type 1 error
...
expected_control_arm <- 90 # number of patients in the control arm
