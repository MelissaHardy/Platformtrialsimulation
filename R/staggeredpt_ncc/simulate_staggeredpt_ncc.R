################################################################################################################
################ R script to simulate multiple Staggered Platform Trials with a nonconcurrent control ##########
################################################################################################################
...
simulate_trials_sptncc <- function(nSims, 
                                n_arms,
  ...
  for (i in seq_len(nSims)) {
    sim <- simulate_full_trial_sptncc(
      n_arms                         = n_arms,
    ...
    # 4b. Store recruitment details and patient‐level data
  ...
  trial_level_means <-  data.frame(
    ...
    overall_platform_power_any = overall_platform_power_any,  # any among effective
    mean_overall_error_rate = mean_platform_errorrate,  # your "platform error rate"
    overall_type1_any    = overall_platform_type1_any,   # any among ineffective
    mse_rd                = mean(mse_per_arm),
    relative_bias = mean(relative_bias_per_arm),
    relative_bias_pct = mean(abs(relative_bias_pct_per_arm)),
    coverage_prob = mean(coverage_by_arm),  # Optional: average of arm-level coverages
    platform_coverage_prob = platform_coverage, # New: platform-wide coverage
