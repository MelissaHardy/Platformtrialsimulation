################################################################################################################
####R Script for simulating outcomes for a Staggered Platform Trial with 5 arms and a non-concurrent control#
################################################################################################################
# This code will simulate a 5-arm non-inferiority randomised controlled staggered platform trial with a non-concurrent
# control arm which will recruit from day 1 with a 1:1 allocation and the other arms will be added at staggered timeframes
# as prespecified in the configuration. It will randomise treatment 0 ("best available therapy") 
...
# Patients receiving treatment 0 and treatment 1-4 will have a probability of mortality that is prespecified
...
simulate_trial_outcomes_sptncc <- function(recruitment_data, 
                                        death_probs, 
                                        ni_margin, 
                                        stagger_months, 
  ...
  for (j in 1:(n_arms - 1)) {
    ...
    # NCC: keep ALL control; restrict Arm j to [start_month, cutoff_month]

    sub <- trial_data %>%
      filter(
        arm == 0 | 
          (arm == arm_index & Month >= start_month & Month <= cutoff_month)
      ) %>%
      mutate(treatment = factor(
        ifelse(arm == 0, "Control", paste0("Arm", arm_index)),
